...
# ans is a local variable that is erased after used.
# global variables are in our environment.
# It's good to remember the "scope" of vars in a
# function don't really exist outside the function.
...
plot_gene_expression <- function(genes_of_interest, df) {
  ...
  ggplot(df_plot_genes, aes(x = Time, y = mean_TPM)) +
    geom_line() +
    geom_errorbar(aes(ymin = mean_TPM - sd_TPM, ymax = mean_TPM + sd_TPM), width = 0.2) + # nolint
    labs(x = 'Time (hours)', y = 'Average TPM') + # nolint
    ggtitle('Average TPM across Time Points') + # nolint
    facet_wrap(~ gene_id)
}
...
#'
#' This function takes a list of genes of interest
#' and DESeq2 results to produce a faceted line plot showing
#' log2 fold changes over a time course. The time points
#' are extracted from the DESeq2 result names,
#' and each gene is plotted separately with its own y-axis scale.
#'
#' @param genes_of_interest
#' A data frame with a column named `gene` containing the gene IDs of interest.
#' @param res_df
#' A data frame containing DESeq2 results with the following columns:
#'   \describe{
...
#'     \item{result_name}{The name of the comparison,
#'                        which should include the time point information.}
#'   }
#'
#' @return
#' # A ggplot object with a faceted line plot of
#' # log2 fold changes over time for each gene.
#'
#' @examples
...
LFCplot <- function(genes_of_interest, res_df) {
  ...
  plot <- ggplot(df_lfc_plot, aes(x = time_point, y = log2FoldChange, group = gene_id)) + # nolint
    geom_line() +
    geom_point() +
