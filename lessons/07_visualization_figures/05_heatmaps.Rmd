...
# load TPM RESULTS:
# load("/scratch/Shares/rinnclass/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/TPM_results/TPM_results.Rdata") # nolint
load("/Users/JasonHunter/Desktop/School/Spring2025/RinnLab/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/TPM_results/TPM_results.rdata") # nolint


# load DESEQ2 results
# load("/scratch/Shares/rinnclass/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/counts_results_DESEQ/DESEQ_results.rdata") # nolint
load("/Users/JasonHunter/Desktop/School/Spring2025/RinnLab/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/counts_results_DESEQ/DESEQ_results.rdata") # nolint
...
# This helps deflate spurious and low values
# - common practice for clustering/heatmaps
log_tpm_matrix <- log2(TPM_filtered + 1)

# Now let's calculate how closely related each sample (time point)
# is to each other using DIST function.
# We make a vector the length of genes in TPM_filtered.
# Then use correlation (DIST function) to find similarity.
# For other methods check out dist menu
?dist

# Let's start with euclidean distance / similarity
# based on log2 TPM values of genes in TPM_filtered.
...
# Downside of ward is it assumes a circle relationship
# where as complete is linear - like we have with time.
hc_complete <- hclust(distance_matrix, method = "complete")

# Now we can use plot() to plot the distance calucalated in hclust!
plot(hc_complete,
     labels = colnames(log_tpm_matrix),
     main = "Global TPM Filtered Dendrogram ",
     sub = "sample relationship by time and replicate after dox treatment")
# Cool we see that samples see to return closer
# to 0 after 96 hours of dox treatment
# Also that the replicates are more similar than time points
# - a very good / expected thing 
...
plot(dend_hc_complete,
     main = "Global TPM Filtered Dendrogram (Ordered by Cluster)",
     sub = "Sample relationship by time and replicate after dox treatment")

# Voila they are ordered now
# - not still takes into consideration similarity when ordering !
...
pheatmap(log_tpm_matrix,
         ...
         scale = "row",        # Scale the data across rows (genes) for better visualization # nolint
...
custom_dist <- function(x) dist(x, method = "manhattan") # Replace "manhattan" with your preferred method # nolint
custom_hclust <- function(x) hclust(x, method = "ward.D2") # Replace "ward.D2" with your preferred clustering method # nolint
...
# Note clustering is just a relative piece of information
...
sig_log_tpm_matrix <- log_tpm_matrix[rownames(log_tpm_matrix) %in% unique_sig_genes, ] # nolint
...
# Plotting
plot(hc_sig_distance_matrix,
     labels = colnames(hc_sig_distance_matrix),
     main = "Log2 TPM Sig Genes Dendrogram ",
     sub = "sample relationship by time and replicate after dox treatment for sig genes") # nolint
...
pheatmap(sig_log_tpm_matrix,
         ...
         show_rownames = FALSE,
         show_colnames = TRUE,
         color = colorRampPalette(c("blue", "white", "red"))(50), # Apply the custom color palette # nolint
...
# Loading DDS
# load("../06_Differential_expression_analyses/results/dds_time_point_mouse_long.RData", verbose = TRUE) # nolint
load("data/dds_time_point_mouse_long.RData", verbose = TRUE) # nolint
...
# Now we retrieve the values using the "assay" function
# that converts to rlog_counts)
rlog_counts_matrix_mouse_long <- assay(rlog_counts_mouse_long) # nolint
# which(is.na(rlog_counts_mouse_long)) # nolint

breaks <- seq(-2, 2, length.out = 51)

# Filter to sig genes
sig_rlog_counts_matrix_mouse_long <- rlog_counts_matrix_mouse_long[rownames(rlog_counts_matrix_mouse_long) %in% unique_sig_genes, ] # nolint
...
pheatmap(sig_rlog_counts_matrix_mouse_long,
  ...
  breaks = breaks,
  color = colorRampPalette(c("blue", "white", "red"))(50) # Gradient from blue to red # nolint
)
...
ordered_matrix <- sig_rlog_counts_matrix_mouse_long[, order(colnames(sig_rlog_counts_matrix_mouse_long))] # nolint
...
pheatmap(average_matrix,
  cluster_rows = TRUE,
  cluster_cols = FALSE,  # Do not cluster columns, just order by time point # nolint
  scale = "row",
  show_rownames = FALSE,
  show_colnames = TRUE,
  main = "Heatmap of Sig genes Rlog-Transformed Values (Averaged by Time Point)", # nolint
  breaks = breaks,
  color = colorRampPalette(c("blue", "white", "red"))(50) # Gradient from blue to red # nolint
)
```
