...
source("/Users/JasonHunter/Desktop/School/Spring2025/RinnLab/MASTER_CLASS/util/00_our_first_function_lesson.R")
```
...
# load("/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/jahu8701/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/TPM_results/TPM_results.rdata") # nolint
...
# load DESEQ2 results
# load("/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/jahu8701/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/counts_results_DESEQ/DESEQ_results.rdata") # nolint
...
hist(average_and_stddev_values$"96_avg",
     ...
     ylim = c(0, 1000),
     breaks = 1000)
# We can quickly see again that most values are very low
# and a long right tail on distribution of TPM values!
```
...
# create an object of time_values grabbing the names of the columns
time_values <- c("0_avg", "12_avg", "24_avg", "48_avg", "96_avg")

# This can also be done with grep (smarter):
time_points <- names(average_and_stddev_values)[grep("avg", names(average_and_stddev_values))]  # nolint
# moving rows into df so we can track gene_id
average_and_stddev_values_2 <- rownames_to_column(average_and_stddev_values, var = "gene_id") # nolint

# Melt the data frame to long format
melt_tpm_df <- melt(average_and_stddev_values_2, measure.vars = time_points, value_name = "gene_id") # nolint


# Cool we just made it a really long df (compare original and melt)
# -it's 5x longer
# Note the same can be done with pivot longer
pivot_longer_tpm_df <- average_and_stddev_values_2 %>% pivot_longer(cols = all_of(time_points), names_to = "Time", values_to = "TPM") # nolint
...
sig_gene_tpm <- inner_join(average_and_stddev_values_2, sig_genes, by = "gene_id") # nolint

# Same can be done by indexing
sig_gene_tpm_2 <- average_and_stddev_values_2[average_and_stddev_values_2$gene_id %in% sig_genes$gene_id, ] # nolint

# let's now merge in the gene names into this handy dataframe
sig_gene_tpm <- merge(sig_gene_tpm, g2s)

# Now we should organize so we can plot specific genes
# (not mix character and integer)
...
Gm9045_tpm <- sig_gene_tpm[sig_gene_tpm$gene_name %in% gene_up$gene_name, ] # nolint

# now moving the character gene_name
# to rownames too won't interfere with numeric values
rownames(Gm9045_tpm) <- Gm9045_tpm$gene_name # nolint

# removing text column
Gm9045_tpm <- Gm9045_tpm %>% select(-gene_name) # nolint
...
df_plot_genes <- TPM_filtered[rownames(TPM_filtered) %in% genes_of_interest$gene, ] # nolint
...
# We also just need to set the parameters
# for the funciton that need to be in env

# First name function, inputs and then the funciton items
# - kinda like shell script
plot_gene_expression <- function(genes_of_interest, df) {
  ...
  # Pivot longer
  df_plot_genes <- df_plot_genes %>%
    pivot_longer(cols = -gene_id, # nolint
  ...
  df_plot_genes <- df_plot_genes %>%
    group_by(gene_id, Time) %>% # nolint
    summarise(mean_TPM = mean(TPM), sd_TPM = sd(TPM)) # nolint

  # Plot the line plot with error bars
  ggplot(df_plot_genes, aes(x = Time, y = mean_TPM)) + # nolint
    geom_line() +
    geom_errorbar(aes(ymin = mean_TPM - sd_TPM, ymax = mean_TPM + sd_TPM), width = 0.2) + # nolint
    labs(x = "Time (hours)", y = "Average TPM") +
    ggtitle("Average TPM across Time Points") +
...
lfc_plot <- function(genes_of_interest, res_df) {
  library(dplyr)
  library(ggplot2)
  ...
  # Remove the "time_point_" prefix and "_vs_0" suffix, then convert to numeric
  df_lfc_plot <- df_lfc_plot %>%
    mutate(time_point = gsub("time_point_|_vs_0", "", result_name)) %>% # nolint
    mutate(time_point = as.numeric(time_point)) # nolint

  # Ensure the data is sorted by gene and time_point
  df_lfc_plot <- df_lfc_plot %>%
    arrange(gene_id, time_point) # nolint
...
lfc_plot(genes_of_interest, res_df)
```
