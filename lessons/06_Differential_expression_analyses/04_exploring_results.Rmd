...
load("/Users/JasonHunter/Desktop/MouseData_Deseq/data/time_point_res_df.RData")
```
...
# cool so we have 669 genes
# that are significantly changing in at least one time point
```
...
# Let's grab the whole row instead of just the value
# (note need to use which.min)
...
write.table(dox_genes["gene_name"],
            row.names = FALSE,
            col.names = FALSE,
            "results/gene_names.csv")
...
basemean_sig_genes <- median(filtered_res_df_2$baseMean)
...
# counts_matrix <- read.table("/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/jahu8701/MASTER_CLASS/data/salmon.merged.gene_counts.tsv", header=TRUE, row.names=1) # nolint
counts_matrix <- read.table("/Users/JasonHunter/Desktop/MouseData_Deseq/results/salmon.merged.gene_counts.tsv",
                            header = TRUE,
                            row.names = 1)
...
# Ok so we see gene_name and gene_id
# are same in both filtered_res_df_2 and counts_matrix
...
1) padj 
```{r}
t.test(res_df$padj, filtered_res_df_2$padj)
```

2) lfcSE
```{r}
t.test(res_df$lfcSE, filtered_res_df_2$lfcSE)
```
3) log2foldChange
```{r}
t.test(res_df$log2FoldChange, filtered_res_df_2$log2FoldChange)
```
...
```{r}
# Filter for genes with padj < 0.01 & absolute fold change > 4 fold (i.e. |log2FC| > 2)
sig_4fold_genes <- res_df %>%
  filter(padj < 0.01, abs(log2FoldChange) > 2)

# Check the result
head(sig_4fold_genes)
summary(sig_4fold_genes)

# Retrieve the counts for these genes from counts_matrix using the gene_id
sig_counts <- counts_matrix[sig_4fold_genes$gene_id, ]

# Check the new counts table
head(sig_counts)

# Save the counts table
write.table(sig_counts, file = "results/sig_4fold_genes_counts.tsv", sep = "\t", quote = FALSE)
```
