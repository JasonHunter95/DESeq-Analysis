...
# counts_matrix <- read.table("/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/jahu8701/MASTER_CLASS/lessons/04_RNAseq_Dox/01_Mouse_dox_wt/dox_long/job_output/star_salmon/salmon.merged.gene_counts.tsv", header=TRUE, row.names=1) # nolint
counts_matrix <- read.table("results/salmon.merged.gene_counts.tsv", header=TRUE, row.names=1) # nolint
...
deseq_samples <- data.frame(sample_id = colnames(counts_matrix))
...
results_names <- resultsNames(dds_time_point)
...
# save(unique_sig_genes, counts_integer, g2s, res_df, filtered_res_df, deseq_samples, file = "/scratch/Shares/rinnclass/MASTER_CLASS/STUDENTS/jahu8701/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/counts_results_DESEQ/DESEQ_results.rdata") # nolint
save(unique_sig_genes, counts_integer, g2s, res_df, filtered_res_df, deseq_samples, file = "/Users/JasonHunter/Desktop/School/Spring2025/RinnLab/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/counts_results_DESEQ/DESEQ_results.rdata") # nolint
...
TPM <- read.table("data/salmon.merged.gene_tpm.tsv", header = TRUE, row.names = 1) # nolint

TPM <- TPM[, -1] # nolint

# Filtering for TPM of at least 1 across samples.
TPM_filtered <- TPM[rowSums(TPM) > 1, ] # nolint
# Checking
...
colnames(average_and_stddev_values) <- paste0(rep(time_points, each = 2), c("_avg", "_sd")) # nolint
```
...
save(TPM, average_and_stddev_values, TPM_filtered, g2s, deseq_samples, file = "/Users/JasonHunter/Desktop/School/Spring2025/RinnLab/MASTER_CLASS/lessons/06_Differential_expression_analyses/results/TPM_results/TPM_results.rdata") # nolint
