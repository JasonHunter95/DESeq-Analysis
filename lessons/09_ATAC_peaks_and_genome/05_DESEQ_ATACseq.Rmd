...
broad_consensus_peaks <- read.table("ATACSeqData/consensus_peaks.mLb.clN.annotatePeaks.txt",
                             sep = "\t", header = TRUE)
...
broad_consensus_counts <- read.table("ATACSeqData/consensus_peaks.mLb.clN.featureCounts.txt",
                             sep = "\t", header = TRUE)
...
```
```{r}
# Lets use the filter function on atac_lfc and atac_lfc_condition - lfc and pval
# Filter for significant peaks in atac_lfc_condition
atac_lfc_condition_sig <- atac_lfc_condition %>%
  filter(padj < 0.05 & abs(log2FoldChange) > 1)
# Let's look at the results
view(atac_lfc_condition_sig)
# Filter for significant peaks in atac_lfc
atac_lfc_sig <- atac_lfc %>%
  filter(padj < 0.05 & abs(log2FoldChange) > 1)
# Let's look at the results
view(atac_lfc_sig)
```
# The results show that there are no significant peaks in the atac_lfc_condition data frame
# or the atac_lfc data frame.
